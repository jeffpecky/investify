<script lang="ts">
    ...
    import { formatDateExt, formatCurrency, copy, cn } from '$lib/utils';
    ...
    let referralLink = $derived(`https://ethercore.org/register?ref=${data.user.referralCode}`);
</script>
...
<div class="space-y-4 px-4 py-4 sm:px-6 lg:px-8">
                    ...
                    <Sheet.Content side="right">
                        ...
                        <div class="grid gap-4 py-6">
                            <p class="text-sm text-foreground leading-relaxed bg-muted/50 p-4 rounded-lg border">
                                The accumulation of tokens will serve as the means of accessing the various referral levels available
                                on the system.
                            </p>
                        </div>
                    </Sheet.Content>
    ...
    <!-- Promotional Banner -->
    <a href="#" class="block relative overflow-hidden rounded-xl border bg-card shadow-sm group mt-1 mb-4 h-32 md:h-40 xl:h-48 cursor-pointer">
        <img
    ...
    <div class="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
        ...
        <StatCard title="Bonus Balance" number={formatCurrency(12450)}>
            ...
            {/snippet}
            {#snippet link()}
                <a href="/referrals" class="text-xs font-medium text-primary hover:underline flex items-center justify-end gap-1 mt-2">
                    Withdraw Bonus
    ...
    <!-- Table -->
    <Card.Root class="py-0 overflow-hidden mt-3">
        <Table.Root class="overflow-hidden">
            ...
            <Table.Body>
                {#each data.referrals as referral}
                    <Table.Row class="[&>td]:py-3 [&>td]:px-4 hover:bg-muted/30 transition-colors">
                        ...
                        <Table.Cell class="font-semibold tabular-nums text-green-600 dark:text-green-500">
                ...
                {:else}
                    <Table.Row>
                        <Table.Cell colspan={7} class="h-48 text-center bg-card">
                            <div class="flex flex-col items-center justify-center text-muted-foreground w-full py-6">
            ...
            <Table.Footer class="bg-muted/20">
                <Table.Row>
                    <Table.Cell colspan={7} class="text-[13px] text-muted-foreground px-6 py-3">
                        Total Downlines: <span class="font-medium text-foreground">{data.referrals.length || 0}</span>
                    </Table.Cell>
                </Table.Row>
            </Table.Footer>
        </Table.Root>
    ...
    <Pagination.Root count={30} page={1} class="justify-center mt-6">
