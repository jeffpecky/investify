<script lang="ts">
    ...
    let wallets = $state($state.snapshot(data.wallets));
...
<div class="space-y-4 px-4 py-4 sm:px-6 lg:px-8">
    ...
    <div class="grid gap-3 mt-2">
        {#each wallets as wallet (wallet.id)}
            ...
            <div
                class="group flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-card rounded-xl border shadow-sm hover:shadow-md transition-all gap-3"
            >
                <!-- Left: Wallet Details -->
                <div class="flex items-center gap-3 min-w-0">
                    <!-- Crypto Icon/Badge -->
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary font-bold text-base">
                ...
                <!-- Right: Stats & Actions -->
                <div class="flex items-center justify-between sm:justify-end gap-4 border-t sm:border-t-0 pt-3 sm:pt-0 mt-1 sm:mt-0">
                    <!-- Received Stats -->
                    <div class="text-left sm:text-right">
                        <p class="text-xs text-muted-foreground mb-1">Total Received</p>
                        <p class="font-semibold tabular-nums text-sm {hasReceived ? 'text-green-600 dark:text-green-400' : 'text-foreground'}">
                    ...
                    <div class="flex items-center gap-2">
                        <Button
                            variant="outline"
                            size="sm"
                            class="h-9 w-9 p-0 text-muted-foreground hover:text-primary transition-colors"
                        ...
                        <Button
                            variant="outline"
                            size="sm"
                            class="h-9 w-9 p-0 text-muted-foreground hover:text-destructive hover:border-destructive/30 hover:bg-destructive/10 transition-colors"
